...
## Testing Guidelines
There is no dedicated framework yet, so document manual verification (`yarn setup` output, light state changes) in every PR. New tests should sit next to the modules under `server/` as `*.spec.ts`, run via Node's built-in `node --test`, and mock Hue endpoints so CI avoids hardware.
